...
export function formatRetrievalDiagnostics(diag: RetrievalDiagnostics): string {
  ...
  const header = [
    ...
    `  removed by relevance gate:   ${diag.relevanceExcludedCount}`,
    `  selected for Opus:           ${diag.selectedCount}`,
    `  web-fallback records:        ${diag.webFallbackCount}`,
    "",
    "CLINICAL-TRIAL NCT IDs FROM SELECTED LITERATURE",
    `  extracted:  ${diag.nctIdsExtracted.length ? diag.nctIdsExtracted.join(", ") : "(none)"}`,
    `  resolved (direct CT.gov record merged): ${diag.nctResolved.length ? diag.nctResolved.join(", ") : "(none)"}`,
    `  unverified (referenced but not in registry): ${diag.nctUnverified.length ? diag.nctUnverified.join(", ") : "(none)"}`,
    "",
